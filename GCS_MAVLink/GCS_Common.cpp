...
ap_message GCS_MAVLINK::mavlink_id_to_ap_message_id(const uint32_t mavlink_id) const
{
    ...
    static const struct {
        ...
        { MAVLINK_MSG_ID_STATUSTEXT,            MSG_STATUSTEXT},
...
bool GCS_MAVLINK::set_ap_message_interval(enum ap_message id, uint16_t interval_ms)
{
    ...
    if (closest_bucket_interval_delta != 0 && empty_bucket_id != -1) {
...
void GCS_MAVLINK::send_message(enum ap_message id)
{
    if (id == MSG_HEARTBEAT) {
        ...
        if (chan == 1 || chan == 3 || chan == 4) {
            const char* status_message = "Mensaje HEARTBEAT RECIBIDO";
            char truncated_message[51] = {}; // Espacio para truncar
            strncpy(truncated_message, status_message, 50); // Truncar a 50 caracteres
            truncated_message[50] = '\0'; // Garantizar terminación nula
            
            mavlink_msg_statustext_send(
                chan,                    // Canal actual
                MAV_SEVERITY_INFO,       // Severidad del mensaje
                truncated_message,           // Texto del mensaje
                0,              // ID del mensaje (opcional)
                0                // Secuencia del fragmento (para mensajes largos)
            );
            gcs().send_text(MAV_SEVERITY_INFO, "Mensaje enviado");
        }
    } else if (id == MSG_STATUSTEXT && (chan == 1 || chan == 3 || chan == 4)) {
        save_signing_timestamp(false);
        
        const char* status_message = "Mensaje STATUS TEXT RECIBIDO";
        char truncated_message[51] = {}; // Espacio para truncar
        strncpy(truncated_message, status_message, 50); // Truncar a 50 caracteres
        truncated_message[50] = '\0'; // Garantizar terminación nula
...
void GCS_MAVLINK::handle_statustext(const mavlink_message_t &msg) const {
    ...
    const char* type = "STATUSTEXT";
    if (msg.sysid != 200){
        // Mostrar el texto del mensaje junto con el puerto
        gcs().send_text(MAV_SEVERITY_INFO, "Puerto: %u - Texto: %s", chan, packet.text);

        if(strcmp(packet.text, "Mensaje con Mavlink UART6") == 0){
            gcs().send_text(MAV_SEVERITY_INFO, "Se recibió %s en port: %u", type, chan);
            // Crear y enviar un mensaje statustext
            const char *response_text = "Mensaje enviado UART6";
            sendMAVLinkMessage(chan, response_text);
        } else if(strcmp(packet.text, "Mensaje con Mavlink UART1") == 0){
            gcs().send_text(MAV_SEVERITY_INFO, "Se recibió %s en puerto: %u", type, chan);
            const char *response_text = "Mensaje enviado UART1";
            sendMAVLinkMessage(chan, response_text);
        } 
    }
}

void GCS_MAVLINK::sendMAVLinkMessage(mavlink_channel_t channel, const char *text) const {
    // Limitar el texto al tamaño permitido por MAVLink
    char truncated_text[MAVLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN + 1] = {};
    strncpy(truncated_text, text, MAVLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN);
    truncated_text[MAVLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN] = '\0'; // Asegurar terminación nula

    // Construir y enviar el mensaje statustext
    mavlink_msg_statustext_send(
        channel,                  // Canal MAVLink
        MAV_SEVERITY_INFO,     // Severidad del mensaje
        truncated_text,        // Texto del mensaje
        0,                     // ID del mensaje (opcional)
        0                      // Fragmento del mensaje (opcional)
    );

    gcs().send_text(MAV_SEVERITY_INFO, "Se envió mensaje MAVLink: %s", truncated_text);
}
...
void GCS_MAVLINK::handle_heartbeat(const mavlink_message_t &msg) const
{
    ...
    const char* type = "HEARTBEAT";
    if (msg.sysid != 200 && chan != 0){
        // Mostrar el texto del mensaje junto con el puerto
        gcs().send_text(MAV_SEVERITY_INFO, "Mensaje tipo %s, Puerto: %u", type, chan);
    }              
}
