...
void Sub::transform_manual_control_to_rc_override(int16_t x, int16_t y, int16_t z, int16_t r, uint16_t buttons)
{
    ...
    float rpyScale = 0.4*gain*ganancia_rotacional*0.01; // Scale -1000-1000 to -400-400 with gain
    float horizontalScale = 0.4*gain*ganancia_horizontal*0.01; // Scale -1000-1000 to -400-400 with gain
    float throttleScale = 0.8*gain*g.throttle_gain*ganancia_vertical*0.01; // Scale 0-1000 to 0-800 times gain
    ...
    if (roll_pitch_flag == 0) {
        ...
        RC_Channels::set_override(4, constrain_int16((xTot)*horizontalScale+rpyCenter,1100,1900), tnow); // forward for ROV
        RC_Channels::set_override(5, constrain_int16((yTot)*horizontalScale+rpyCenter,1100,1900), tnow); // lateral for ROV
    } else {
        // neutralize forward and lateral input while we are adjusting roll and pitch
        RC_Channels::set_override(4, constrain_int16(xTrim*horizontalScale+rpyCenter,1100,1900), tnow); // forward for ROV
        RC_Channels::set_override(5, constrain_int16(yTrim*horizontalScale+rpyCenter,1100,1900), tnow); // lateral for ROV
    }
...
void Sub::handle_jsbutton_release(uint8_t _button, bool shift) {
    ...
    case JSButton::button_function_t::k_custom_1:
        presion_boton1 = false;
        break;
    case JSButton::button_function_t::k_custom_2:
        presion_boton2 = false;
        break;
    case JSButton::button_function_t::k_custom_3:
        presion_boton3 = false;
        break;
    case JSButton::button_function_t::k_custom_4:
        presion_boton4 = false;
        break;
    case JSButton::button_function_t::k_custom_5:
        presion_boton5 = false;
        break;
    case JSButton::button_function_t::k_custom_6:
        presion_boton6 = false;
        break;
    }
}
...
void Sub::abrir_garra_izquierda(bool activo,bool sostenido, bool soltado){
    if(activo){
        SRV_Channel* chan = SRV_Channels::srv_channel(SERVO_CHAN_1 - 1); // 0-indexed
        ServoRelayEvents.do_set_servo(SERVO_CHAN_1, chan->get_output_max()); // 1-indexed
        relay.on(0);
...
void Sub::abrir_garra_derecha(bool activo,bool sostenido, bool soltado){
    if(activo){
        SRV_Channel* chan = SRV_Channels::srv_channel(SERVO_CHAN_1 - 1); // 0-indexed
        ServoRelayEvents.do_set_servo(SERVO_CHAN_1, chan->get_output_max()); // 1-indexed
        relay.on(1);
...
void Sub::cerrar_garra(bool activo,bool sostenido, bool soltado){
    if(activo){
        SRV_Channel* chan = SRV_Channels::srv_channel(SERVO_CHAN_1 - 1); // 0-indexed
        ServoRelayEvents.do_set_servo(SERVO_CHAN_1, chan->get_output_min()); // 1-indexed
    }else if(soltado){
...
void Sub::subir_camara(bool activo,bool sostenido, bool soltado){
    if(activo){
        cam_tilt = 1100;
    }else if(soltado){
        cam_tilt = 1500;
...
void Sub::bajar_camara(bool activo,bool sostenido, bool soltado){
    if(activo){
        cam_tilt = 1900;
    }else if(soltado){
        cam_tilt = 1500;
...
void Sub::aumentar_luz(bool activo,bool sostenido, bool soltado){
    if (activo && !sostenido) {
        RC_Channel* chan = RC_Channels::rc_channel(8);
        uint16_t min = chan->get_radio_min();
        uint16_t max = chan->get_radio_max();
...
void Sub::disminuir_luz(bool activo,bool sostenido, bool soltado){
    if (activo && !sostenido) {
        RC_Channel* chan = RC_Channels::rc_channel(8);
        uint16_t min = chan->get_radio_min();
        uint16_t max = chan->get_radio_max();
...
void Sub::aumentar_ganancia(bool activo,bool sostenido, bool soltado){
    if (activo && !sostenido) {
        // check that our gain parameters are in correct range, update in eeprom and notify gcs if needed
        g.minGain.set_and_save(constrain_float(g.minGain, 0.10, 0.80));
        g.maxGain.set_and_save(constrain_float(g.maxGain, g.minGain, 1.0));
...
void Sub::disminuir_ganancia(bool activo,bool sostenido, bool soltado){
    if (activo && !sostenido) {
        // check that our gain parameters are in correct range, update in eeprom and notify gcs if needed
        g.minGain.set_and_save(constrain_float(g.minGain, 0.10, 0.80));
        g.maxGain.set_and_save(constrain_float(g.maxGain, g.minGain, 1.0));
...
void Sub::aumentar_ganancia_vertical(bool activo,bool sostenido, bool soltado){
    if(activo && !sostenido){
        ganancia_vertical=constrain_int16(ganancia_vertical+20, 20, 100);
        gcs().send_text(MAV_SEVERITY_INFO,"#Ganancia vertical: %d%%",ganancia_vertical);
    }
}
void Sub::disminuir_ganancia_vertical(bool activo,bool sostenido, bool soltado){
    if(activo && !sostenido){
        ganancia_vertical=constrain_int16(ganancia_vertical-20, 20, 100);
        gcs().send_text(MAV_SEVERITY_INFO,"#Ganancia vertical: %d%%",ganancia_vertical);
    }
}
void Sub::aumentar_ganancia_horizontal(bool activo,bool sostenido, bool soltado){
    if(activo && !sostenido){
        ganancia_horizontal=constrain_int16(ganancia_horizontal+20, 20, 100);
        gcs().send_text(MAV_SEVERITY_INFO,"#Ganancia horizontal: %d%%",ganancia_horizontal);
    }
}
void Sub::disminuir_ganancia_horizontal(bool activo,bool sostenido, bool soltado){
    if(activo && !sostenido){
        ganancia_horizontal=constrain_int16(ganancia_horizontal-20, 20, 100);
        gcs().send_text(MAV_SEVERITY_INFO,"#Ganancia horizontal: %d%%",ganancia_horizontal);
    }
}
void Sub::aumentar_ganancia_rotacional(bool activo,bool sostenido, bool soltado){
    if(activo && !sostenido){
        ganancia_rotacional=constrain_int16(ganancia_rotacional+20, 20, 100);
        gcs().send_text(MAV_SEVERITY_INFO,"#Ganancia rotacional: %d%%",ganancia_rotacional);
    }
}
void Sub::disminuir_ganancia_rotacional(bool activo,bool sostenido, bool soltado){
    if(activo && !sostenido){
        ganancia_rotacional=constrain_int16(ganancia_rotacional-20, 20, 100);
        gcs().send_text(MAV_SEVERITY_INFO,"#Ganancia rotacional: %d%%",ganancia_rotacional);
    }
